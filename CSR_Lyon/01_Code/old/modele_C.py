...
def run_scs_hsm(dt_obs,
    ...
    r_out = np.zeros(nt)      # m/s (vers exutoire)
...
def compute_mass_balance_B(res: dict,
                           ...
                           h_r0: float = 0.0) -> dict:
    """
    Bilan de masse au pas obs (dt_obs) pour le code B.

    Convention :
      - p_rate, etp_rate : m/s
      - res["r_out"]     : m/s (débit surf vers exutoire)
      - res["seep_loss"] : m par pas obs (volume surfacique)
      - res["sa_loss"]   : m par pas obs (ET sur Ia)
      - Etats : res["h_a"], res["h_s"], res["h_r"] en m
    """
    ...
    # Si jamais tu initialises ailleurs, tu peux passer h_a0, h_s0, h_r0
...
def main():
    ...
    # ---- ETP (Ia uniquement dans le modèle)
    ETP_MM_PER_DAY = 5
    ...
    # ---- bornes k_infiltr en mm/h (si tu calibres encore)
    KINF_MIN_MM_H = 0.03
    KINF_MAX_MM_H = 0.35
